...
def split_data():
    x, y, s = load_data()
    #data split
    x_train, x_test, y_train, y_test = train_test_split(x, y,train_size = 1-0.2, test_size = 0.2, random_state =7)
...
def baseline_model():
	...
	# ajouter des couches
	model.add(Dense(64, input_dim=135, activation='relu'))
...
def main():
    ...
    (X_num_tr, y_tr), (X_num_val, y_val), x,y, Class = split_data()
    ...
    #print("jhvutdxrcfvgbhunj",pred.shape)
    print(pred[0:10])
    print(y_val[0:10])
    ...
    # to convert the predictions to the expected classes.  The argmax function finds the index of the maximum prediction
    # for each row.
    ...
    print(Class[predict_classes[0:5]])

    # Accuracy  is essentially a test score.  For all of the iris predictions,
