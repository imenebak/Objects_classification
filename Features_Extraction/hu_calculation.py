...
def degismezMoment(p,q):
    ...
    for x in range (imageWidth):
        for y in range(imageHeight):
                degismezMomentVar = pow(x-CentreX,p)*pow(y-CentreY,q)*imageArray[y][x] + degismezMomentVar
...
#    log Transform
    ...
    fichier = open(pos+"/Hu.txt", "a")
    fichier.write("\n"+str(normalizeHuMoments[0])+","+str(normalizeHuMoments[1])+","+str(normalizeHuMoments[2])+","+str(normalizeHuMoments[3])+","+str(normalizeHuMoments[4])+","+str(normalizeHuMoments[5])+","+str(normalizeHuMoments[6]))
    fichier.close()

for o in pat:
    pati = str(o)
    debut = pati+"/train"
    fichier = open(pati+"/Hu.txt", "w")
    fichier.write("id,m1,m2,m3,m4,m5,m6,m7")
    fichier.close()
